...
Filtros (INEP + SED):
  - MS, por dependência administrativa
  - Concluintes 2019-2023: TP_ST_CONCLUSAO = 2
  - Concluintes 2024: candidato com escola identificada
  - Presentes nos 2 dias (TP_PRESENCA = 1 em CN, CH, LC, MT)
  - Não eliminados: TP_PRESENCA != 2 e TP_STATUS_REDACAO != 2
  - Redação em branco (status 4) permanece na população válida
  - Denominador de concluintes: planilha SED (Concluintes EM 2019 a 2024.xlsx)
"""
...
def _cols_parquet() -> list[str]:
    base = [
        "NU_ANO", "NU_INSCRICAO", "SG_UF_ESC", "NO_MUNICIPIO_ESC",
        "TP_DEPENDENCIA_ADM_ESC", "CO_ESCOLA", "TP_ST_CONCLUSAO",
        *PRES_COLS, "TP_STATUS_REDACAO", *COLS_NOTAS,
    ]
    return base
...
def processar_ano(df_ano: pd.DataFrame, cres, mapa_muni, conc_totais, conc_esc) -> dict:
    ...
    for dep in DEPENDENCIAS:
        ...
        elim_obj = int(base["ELIM_OBJ"].sum())
    ...
    # Brasil estadual (referência nacional)
    ...
    if not ms_val.empty and "CRE" in ms_val.columns:
        for dep in DEPENDENCIAS:
            ...
            if dep == "Estadual":
                ...
                if not conc_cre.empty and "CO_ESCOLA" in sub.columns:
                    # concluintes por CRE via escola
    ...
    if ano == 2024:
        ...
        if not sub24.empty and "CO_ESCOLA" in sub24.columns:
    ...
    # quantis por área (MS estadual válidos)
...
def main():
    print("=" * 60)
    print("GERADOR DE AGREGADOS — ENEM MS (filtros INEP corrigidos)")
    ...
    for ano in ANOS:
        print(f"\nProcessando {ano}...")
        df_ano = pd.read_parquet(PARQUET, columns=cols, filters=[("NU_ANO", "==", ano)])
    ...
    print(f"  inscritos: {int(ms['inscritos'])}")
    print(f"  presentes 2d: {int(ms['presentes'])}")
    print(f"  eliminados redacao (st=2): {int(ms['eliminados_redacao'])}")
    print(f"  redacao branco (st=4): {int(ms['redacao_branco'])}")
    print(f"  validos (presfilt): {int(ms['presentes_filt'])}")
    if ms["concluintes"]:
