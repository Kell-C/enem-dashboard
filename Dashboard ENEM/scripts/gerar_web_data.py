"""
Exporta data.json e painel_data.js para o site estático em web/.
"""
...
from enem_helpers import carregar_concluintes_sed, cre_curto, normalizar_texto, quantis_serie

NOTA_COLS = list(NOTA_MAP.values())
REF_AREAS = {
    "CN": "NU_NOTA_CN",
    "CH": "NU_NOTA_CH",
    "LC": "NU_NOTA_LC",
...
def _areas_serie(evol_cre: pd.DataFrame, cre_name: str, dep: str = "Estadual") -> dict:
    ...
    for k in AREA_KEYS:
        ...
        for a in ANOS:
            row = sub[sub["ano"] == a]
            areas[k].append(round(float(row.iloc[0][col]), 1) if not row.empty and col in row.columns and pd.notna(row.iloc[0][col]) else None)
    return areas
...
def _uf_rank_por_ano(des_uf: pd.DataFrame) -> dict:
    ...
    for a in ANOS:
        ...
        out[str(a)] = [
            [str(row["UF"]), round(float(row["media_geral"]), 0)]
...
def build_painel_data() -> dict:
    ...
    tx_ms = [
        round(100 * n / c, 1) if c else None
        ...
        for mname, grp in evol_muni[evol_muni["dependencia"] == "Estadual"].groupby("NO_MUNICIPIO_ESC"):
            ...
            for a in ANOS:
                ...
                if row.empty:
                    ...
                    conc = conc_esc[
                        (conc_esc["NU_ANO"] == a)
                        & (conc_esc["Município"].map(normalizar_texto) == normalizar_texto(mname))
                    ]["Concluintes"].sum()
    ...
    for escopo in ["Estadual", "Federal", "Municipal", "Privada", "Brasil-Estadual"]:
        ...
        if sub.empty and escopo == "Brasil-Estadual":
            sub = integ_df[integ_df["escopo"] == "Brasil-Estadual"]
        if sub.empty:
            continue
        key = escopo
        integ["rede"][key] = {
    ...
    # boxplot / histograma from quantis
    boxplot = {k: _histograma_bins(quantis, k) for k in AREA_KEYS}
    histograma = boxplot.copy()

    # escRank top/bottom 2024
    esc_rank = {k: [] for k in AREA_KEYS}
    if not esc24.empty:
        col_map = {"CN": "media_nu_nota_cn", "CH": "media_nu_nota_ch", "LC": "media_nu_nota_lc",
    ...
    index_areas = {
        k: _serie_refs(refs, REF_AREAS[k], "media_ms")
        for k in AREA_KEYS
    }
    ms_area_series = {
    ...
    tx_elim = [
        float(ms_part[ms_part["ano"] == a].iloc[0]["eliminados_redacao"]) / float(ms_part[ms_part["ano"] == a].iloc[0]["presentes"]) * 100
    ...
    meta = {
        "filtros": {
            ...
            "concluintes_2024": "escola identificada no microdado",
            ...
            "concluintes_denominador": "Concluintes EM 2019 a 2024.xlsx (SED)",
        },
...
def main():
    if not PASTA_AGREGADOS.exists():
        raise SystemExit("Rode primeiro: python scripts/gerar_agregados.py")
    ...
    print(f"MS 2024: {f24['presfilt']} validos / {f24['concluintes']} concluintes SED "
