...
ARQUIVO_CRES = r"C:\Users\User\.verdent\verdent-projects\revise-e-melhore-este-6f3dd0f1\explorando\cres.xlsx"
...
CORES_AREAS = {
    "MEDIA_GERAL":     COR_NEUTRO,         # Média Geral — cinza grafite (composto/agregado)
    "NU_NOTA_CN":      COR_POSITIVO,       # Ciências da Natureza — verde
    "NU_NOTA_CH":      LARANJA_DESTAQUE,   # Ciências Humanas — laranja
    "NU_NOTA_LC":      AZUL_PRINCIPAL,     # Linguagens e Códigos — azul institucional
...
def carregar_base_bruta() -> pd.DataFrame:
    ...
    df["CO_ESCOLA"] = pd.to_numeric(df["CO_ESCOLA"], errors="coerce").astype("Int64")
...
@st.cache_data(show_spinner="Aplicando critérios de inclusão...")
...
@st.cache_data(show_spinner="Carregando cadastro CRES...")
...
def carregar_mapa_municipio_cre() -> dict:
    ...
    for _, row in df_cres.iterrows():
        ...
        if municipio and cre:
            municipio_norm = municipio.replace("Á", "A").replace("É", "E").replace("Í", "I").replace("Ó", "O").replace("Ú", "U").replace("Â", "A").replace("Ê", "E").replace("Î", "I").replace("Ô", "O").replace("Û", "U").replace("Ã", "A").replace("Õ", "O").replace("Ç", "C").replace("À", "A")
            mapa[municipio_norm] = cre
    return mapa
...
def aplicar_tema(fig, altura: int = 400):
    ...
    fig.update_layout(
        ...
        legend=dict(
            ...
            font=dict(size=11.5, color=TEMA["texto"]),
...
def _adicionar_referencias_ms_br(fig, media_ms, media_br, *,
    ...
    - Itens MS e BR compartilham ``legendgroup="medias_ref"`` para toggle
      conjunto.
    """
    tem_ms = media_ms is not None and not (isinstance(media_ms, float) and np.isnan(media_ms))
    tem_br = media_br is not None and not (isinstance(media_br, float) and np.isnan(media_br))
    if not tem_ms and not tem_br:
        return

    modo_paper = x_dominio_min is None or x_dominio_max is None
    proximas = tem_ms and tem_br and abs(float(media_ms) - float(media_br)) < limiar_colisao
...
def _adicionar_series_medias_ms_br(fig, serie_ms, serie_br, *,
                                   sufixo_legenda: str = "rede estadual",
    ...
    """
    def _add(serie, cor, dash, nome, pos_rotulo):
        if serie is None:
            return
        try:
        ...
        if s.empty:
            return
        x_vals = [int(x) if float(x).is_integer() else x for x in s.index.tolist()]
        ...
        fig.add_trace(go.Scatter(
            ...
            line=dict(color=cor, dash=dash, width=2.2),
            marker=dict(color=cor, size=8,
                        line=dict(color="#ffffff", width=1.2)),
            text=textos if mostrar_rotulos else None,
            textposition=pos_rotulo,
            textfont=dict(size=10.5, color=cor,
                          family="Source Sans 3, system-ui, sans-serif"),
            name=f"Média {nome} — {sufixo_legenda}",
            legendgroup="medias_ref",
            hovertemplate=(
                f"<b>Média {nome} — {sufixo_legenda}</b><br>"
    ...
    _add(serie_ms, AZUL_PRINCIPAL, "dash", "MS", "top center")
    _add(serie_br, COR_BRASIL, "dot", "BR", "bottom center")
...
def diagnostico_estadual(df_filt_ms, df_bruta_ms, df_br_filt) -> dict:
    ...
    d["n_part"] = len(df_est)
    d["tx_part"] = round(100 * d["n_part"] / d["n_inscritos"], 1) if d["n_inscritos"] else 0.0
    d["media_estadual_ms"] = float(df_est["MEDIA_GERAL"].mean()) if not df_est.empty else np.nan
    d["media_estadual_br"] = float(df_est_br["MEDIA_GERAL"].mean()) if not df_est_br.empty else np.nan
...
_STOP_ABR     = {"DE", "DA", "DO", "DAS", "DOS", "E", "EM", "A", "O", "AS", "OS", "NO", "NA"}
...
def fig_participacao_por_ano(df_bruta_ms, df_filt_ms, anos_sel, dep="Estadual"):
    ...
    for ano in sorted(anos_sel):
        insc = len(df_bruta_ms[(df_bruta_ms["NU_ANO"] == ano) & (df_bruta_ms["DEP_ADM"] == dep)])
        part = len(df_filt_ms[(df_filt_ms["NU_ANO"] == ano) & (df_filt_ms["DEP_ADM"] == dep)])
        pct = round(100 * part / insc, 1) if insc else 0.0
        linhas.append(dict(Ano=int(ano), Inscritos=insc, Participantes=part, Pct=pct))
...
def fig_evolucao_area(df, col, deps, titulo, df_contexto=None,
    ...
    if df_contexto is not None and not df_contexto.empty:
        s_br = df_contexto.groupby("NU_ANO", observed=True)[col].mean().reset_index()
        fig.add_trace(go.Scatter(
    ...
    if media_ms_ref is not None:
        fig.add_hline(y=media_ms_ref, line_dash="dash", line_color=AZUL_PRINCIPAL,
                      annotation_text="Média MS", annotation_position="top left")
    if media_br_ref is not None:
        fig.add_hline(y=media_br_ref, line_dash="dot", line_color=COR_BRASIL,
                      annotation_text="Média BR", annotation_position="bottom left")
    ...
    if df_contexto is not None and not df_contexto.empty:
        series_eixo.append(df_contexto.groupby("NU_ANO", observed=True)[col].mean())
    fig.update_layout(
        title=titulo,
...
def fig_uf_barras(df, col, dep_filtro, titulo):
    ...
    cores = [LARANJA_DESTAQUE if uf == "MS" else COR_BAR_NEUTRA for uf in g["UF"]]
    fig = go.Figure(go.Bar(
...
def aba_sumario_executivo(diag: dict, anos_sel: list):
    ...
    status_pos = classificar_posicao(diag.get("pos_ms"), diag.get("total_ufs", 0))
    ...
    with col_p:
        if status_part == "positivo":
            achado("positivo", "Alta participação",
                   f"{fmt_pct(diag['tx_part'])} dos inscritos compareceram aos dois dias.")
        if status_var == "positivo":
            achado("positivo", "Crescimento no período",
                   f"A média subiu {fmt_delta(diag['variacao_inicio_fim'])}.")
    with col_a:
        if status_part == "atencao":
            achado("atencao", "Participação intermediária",
                   f"Taxa de {fmt_pct(diag['tx_part'])} sugere margem para melhoria.")
        if status_var == "atencao":
            achado("atencao", "Estabilidade",
                   f"Variação de {fmt_delta(diag['variacao_inicio_fim'])} indica estabilidade.")
    with col_c:
        if status_part == "critico":
            achado("critico", "Baixa participação",
                   f"Apenas {fmt_pct(diag['tx_part'])} compareceram aos dois dias.")
        if status_var == "critico":
            achado("critico", "Queda no período",
                   f"A média caiu {fmt_delta(diag['variacao_inicio_fim'])}.")
...
def aba_panorama_participacao(df_bruta_ms, df_filt_ms, anos_sel,
    ...
    titulo_secao(
        ...
        "Adesão por ano e por Coordenadoria Regional de Educação."
    )

    _chart(fig_participacao_por_ano(df_bruta_ms, df_filt_ms, anos_sel, "Estadual"))

    # --- Gráfico de barras verticais por CRE (semáforo de participação, facetas por ano) ---
    df_est_enriq = df_filt_ms_enriq[df_filt_ms_enriq["DEP_ADM"] == "Estadual"] if df_filt_ms_enriq is not None else None
    df_est_bruta_enriq = df_bruta_ms_enriq[df_bruta_ms_enriq["DEP_ADM"] == "Estadual"] if df_bruta_ms_enriq is not None else None

    if df_est_enriq is not None and "CRE" in df_est_enriq.columns and df_est_bruta_enriq is not None and "CRE" in df_est_bruta_enriq.columns:
        titulo_secao(
            "Taxa de participação por CRE — rede estadual",
            "Cores indicam o nível de adesão; ordem das CREs mantida entre os anos para comparação direta."
        )

        def _classifica_taxa(taxa):
            if pd.isna(taxa):
                return (TEMA["texto_secundario"], "Sem dados")
            if taxa >= 80:
                return (COR_POSITIVO, "Positivo (≥ 80%)")
            if taxa >= 60:
                return (COR_ATENCAO, "Atenção (60–79%)")
            return (COR_CRITICO, "Crítico (< 60%)")

        linhas_cre = []
        anos_cre = sorted(anos_sel)
        cres_list = sorted(df_est_enriq.dropna(subset=["CRE"])["CRE"].unique())

        for ano in anos_cre:
            for cre in cres_list:
                insc = len(df_est_bruta_enriq[(df_est_bruta_enriq["NU_ANO"] == ano) & (df_est_bruta_enriq["CRE"] == cre)])
                part = len(df_est_enriq[(df_est_enriq["NU_ANO"] == ano) & (df_est_enriq["CRE"] == cre)])
                if insc > 0:
                    pct = round(100 * part / insc, 1)
                else:
                    pct = float("nan")
                linhas_cre.append(dict(Ano=str(ano), CRE=cre, Inscritos=int(insc), Participantes=int(part), Taxa=pct))

        if linhas_cre:
            df_cre = pd.DataFrame(linhas_cre)
            n_anos = len(anos_cre)

            ano_ref = max(anos_cre)
            d_ref = df_cre[(df_cre["Ano"] == str(ano_ref)) & df_cre["Taxa"].notna()]
            if not d_ref.empty:
                ordem_cres = d_ref.sort_values("Taxa", ascending=False)["CRE"].tolist()
                cres_restantes = [c for c in cres_list if c not in ordem_cres]
                ordem_cres = ordem_cres + cres_restantes
            else:
                medias = (df_cre.groupby("CRE")["Taxa"].mean(numeric_only=True)
                          .sort_values(ascending=False, na_position="last"))
                ordem_cres = medias.index.tolist()

            fig_cres = make_subplots(
                rows=n_anos, cols=1,
                subplot_titles=[f"Ano {a}" for a in anos_cre],
                shared_xaxes=True,
                vertical_spacing=0.08,
            )

            for idx, ano in enumerate(anos_cre, start=1):
                d_ano = df_cre[df_cre["Ano"] == str(ano)].copy()
                d_ano["CRE"] = pd.Categorical(d_ano["CRE"], categories=ordem_cres, ordered=True)
                d_ano = d_ano.sort_values("CRE")
                cores_barras = [_classifica_taxa(t)[0] for t in d_ano["Taxa"]]
                rotulos = [f"{t:.1f}%" if pd.notna(t) else "—" for t in d_ano["Taxa"]]
                fig_cres.add_trace(
                    go.Bar(
                        x=d_ano["CRE"].astype(str),
                        y=d_ano["Taxa"],
                        ...
                        cliponaxis=False,
                        showlegend=False,
                        customdata=d_ano[["Inscritos", "Participantes"]].values,
                        hovertemplate=(
                            "<b>%{x}</b><br>"
                            f"Ano: {ano}<br>"
                            "Taxa: %{y:.1f}%<br>"
                            "Participantes: %{customdata[1]:,}<br>"
                            "Inscritos: %{customdata[0]:,}"
                            "<extra></extra>"
                        ).replace(",", "."),
                    ),
                    row=idx, col=1,
                )
                fig_cres.update_yaxes(range=[0, 105], ticksuffix="%", row=idx, col=1)
                fig_cres.update_xaxes(
                    type="category",
                    categoryorder="array",
                    categoryarray=ordem_cres,
                    tickangle=-30,
                    row=idx, col=1,
                )
                fig_cres.add_hline(
                    y=80,
                    line_dash="dash",
                    line_color=TEMA["texto_secundario"],
                    line_width=1,
                    annotation_text="Meta 80%",
                    annotation_position="top right",
                    annotation_font_size=10,
                    annotation_font_color=TEMA["texto_secundario"],
                    row=idx, col=1,
                )

            for cor, rotulo in [
                (COR_POSITIVO, "Positivo (≥ 80%)"),
                (COR_ATENCAO,  "Atenção (60–79%)"),
                (COR_CRITICO,  "Crítico (< 60%)"),
            ]:
                fig_cres.add_trace(
                    go.Bar(
                        x=[None], y=[None],
            ...
            altura_cre = max(360, 240 * n_anos)
            fig_cres.update_layout(
                height=altura_cre,
                bargap=0.25,
            )
            fig_cres = aplicar_tema(fig_cres, altura_cre)
            fig_cres.update_layout(
            ...
            for ann in fig_cres.layout.annotations:
                if ann.text and ann.text.startswith("Ano "):
                    ann.font = dict(size=12, color=TEMA["texto"], family="Plus Jakarta Sans, sans-serif")
            _chart(fig_cres)
        else:
            st.info("Nenhuma CRE encontrada nos dados.")
    else:
        st.info("Dados de CRE não disponíveis para o gráfico de participação.")

    # --- Gráfico de posição nacional do MS ---
    if df_bruta_nacional is not None and df_filt_nacional is not None:
        st.markdown("### Posição do MS na taxa de participação nacional (rede estadual)")
        col_uf_nac = "SG_UF_ESC" if "SG_UF_ESC" in df_bruta_nacional.columns else "SG_UF_PROVA"
        posicoes_ms = []
        for ano in sorted(anos_sel):
            df_br_ano = df_bruta_nacional[(df_bruta_nacional["NU_ANO"] == ano) & (df_bruta_nacional["DEP_ADM"] == "Estadual")]
            df_fi_ano = df_filt_nacional[(df_filt_nacional["NU_ANO"] == ano) & (df_filt_nacional["DEP_ADM"] == "Estadual")]
            taxas_uf = []
            ...
            insc_ms = len(df_bruta_ms[(df_bruta_ms["NU_ANO"] == ano) & (df_bruta_ms["DEP_ADM"] == "Estadual")])
            part_ms = len(df_filt_ms[(df_filt_ms["NU_ANO"] == ano) & (df_filt_ms["DEP_ADM"] == "Estadual")])
            taxa_ms = round(100 * part_ms / insc_ms, 1) if insc_ms else 0.0
            if "MS" in ranking:
                pos_ms = ranking.index("MS") + 1
                posicoes_ms.append(dict(Ano=ano, Posição=pos_ms, Total=len(ranking), Taxa=taxa_ms))
        ...
        if posicoes_ms:
            ...
            terco = n_total / 3
            fig_pos.add_hrect(y0=0, y1=terco, fillcolor=COR_POSITIVO, opacity=0.08, line_width=0)
            fig_pos.add_hrect(y0=terco, y1=2*terco, fillcolor=COR_ATENCAO, opacity=0.08, line_width=0)
            fig_pos.add_hrect(y0=2*terco, y1=n_total+1, fillcolor=COR_CRITICO, opacity=0.08, line_width=0)
            ...
            fig_pos.add_trace(go.Scatter(
                ...
                mode="lines+markers+text",
                text=[f"{int(r['Posição'])}º de {int(r['Total'])} ({r['Taxa']:.1f}%)" for _, r in df_pos.iterrows()],
                textposition="top center",
                line=dict(color=AZUL_PRINCIPAL, width=3),
                marker=dict(size=12, color=cores, line=dict(width=2, color="white")),
            ))
            fig_pos.update_yaxes(autorange="reversed", title="Posição (1º = melhor)")
    ...
    taxa_est = round(100 * total_part_est / total_insc_est, 1) if total_insc_est else 0
    insight_box(
...
def aba_desempenho(df_filt_ms, df_br_filt):
    ...
    df_est_br = df_br_filt[df_br_filt["DEP_ADM"] == "Estadual"]
    deps_exibir = [d for d in ORDEM_DEP if d in df_filt_ms["DEP_ADM"].dropna().unique()]
    ...
    for ano in anos:
        for key in areas_keys:
            ...
            delta = media_ms - media_br if (pd.notna(media_ms) and pd.notna(media_br)) else float("nan")
            dados.append({
                "Ano": int(ano),
                "AreaKey": key,
    ...
    if df_plot.empty or df_plot["MediaMS"].dropna().empty:
        ...
        serie_global = pd.concat([df_plot["MediaMS"], df_plot["MediaBR"]]).dropna()
        if not serie_global.empty:
            amp_g = float(serie_global.max() - serie_global.min())
            y_min_global = max(0, float(serie_global.min()) - max(15, 0.12 * (amp_g + 1)))
            y_max_global = min(1000, float(serie_global.max()) + max(25, 0.18 * (amp_g + 1)))
        else:
        ...
        for i, key in enumerate(areas_keys):
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    textposition="top center",
                    textfont=dict(size=10, color=TEMA["texto"], family="Plus Jakarta Sans, sans-serif"),
            ...
            if not d_valid.empty:
                ...
                fig.add_annotation(
                    ...
                    yshift=26,
                    xanchor="center",
                    font=dict(size=11, color=cor_d, family="Plus Jakarta Sans, sans-serif"),
        ...
        for ann in fig.layout.annotations:
            if ann.text and ann.text in nomes_subtitulos:
                ann.font = dict(size=12.5, color=TEMA["texto"], family="Plus Jakarta Sans, sans-serif")
    ...
    if df_est_ms.empty or df_est_ms["NU_ANO"].dropna().empty:
        ...
        series_por_area = [df_box_tmp[df_box_tmp[col] > 0][col] for col in COLS_NOTAS]
    ...
    if df_est_ms.empty or df_est_ms["NU_ANO"].dropna().empty:
        ...
        for key in areas_keys:
            ...
            for ano in anos_int:
                ...
                _std  = float(notas_ano.std(ddof=1)) if len(notas_ano) > 1 else 0.0
                registros.append({
                    "Ano": ano,
                    "Media": _mean,
        ...
        if not todos_lo.empty and not todos_hi.empty:
            ...
            y_min_fan = max(0.0, float(todos_lo.min()) - max(15, 0.08 * (amp_fan + 1)))
            y_max_fan = min(1000.0, float(todos_hi.max()) + max(20, 0.10 * (amp_fan + 1)))
        else:
        ...
        for ann in fig_fan.layout.annotations:
            if ann.text and ann.text in nomes_subtitulos:
                ann.font = dict(size=12.5, color=TEMA["texto"], family="Plus Jakarta Sans, sans-serif")
    ...
    if d_ms_area.empty:
        ...
        # Um trace go.Box por ano, com valores brutos para que Plotly renderize
        # apenas os outliers (boxpoints="outliers"). x=[ano]*N alinha cada
        # ponto ao mesmo bin categórico do ano.
        ...
        # Afirma eixo X categórico ANTES dos overlays Scatter (anti-quirk Plotly)
        ...
        # Sobreposição das curvas de média MS e BR ano a ano
        # (linhas+marcadores+rótulos — variação ao longo dos anos)
        _adicionar_series_medias_ms_br(
            fig_box_area,
            serie_ms_anual if not serie_ms_anual.empty else None,
            serie_br_anual if not serie_br_anual.empty else None,
            sufixo_legenda="rede estadual",
            mostrar_rotulos=True,
        )

        # Range Y considera valores do boxplot + ambas as séries de médias
        series_para_range = [d_ms_area[d_ms_area["AnoCat"] == a][area_sel]
                             for a in anos_box_area]
    ...
    if anos_presentes:
        ...
        media_ms_ano = df_ult[df_ult["DEP_ADM"] == "Estadual"][area_sel].mean()
        media_br_ano = df_est_br[df_est_br["NU_ANO"] == ano_comparacao][area_sel].mean()
        ...
        _adicionar_referencias_ms_br(
            fig_box_areas, media_ms_ano, media_br_ano,
            sufixo_legenda="rede estadual",
        )
        ...
        fig_box_areas.update_layout(
            ...
            yaxis=dict(title="Nota",
                       range=range_dinamico(*_series_box, padding=0.05,
                                            referencias=(media_ms_ano, media_br_ano))),
            xaxis=dict(title="Área de conhecimento"),
        )
        st.markdown(
            _mini_legenda_medias_html(media_ms_ano, media_br_ano),
            unsafe_allow_html=True,
        )
...
def aba_escolas_2024(df_ms_enriq_2024, ano=2024, df_br=None, df_bruta_ms=None):
    ...
    y_range_escolas = range_dinamico(
        dados_top[area], dados_bot[area],
        padding=0.05,
        referencias=(medias_ref["ms"], medias_ref["br"]),
    )
    ...
    with col_box1:
        if not dados_top.empty:
            ...
            _chart(aplicar_tema(fig_top_box, 560))
        else:
    ...
    with col_box2:
        if not dados_bot.empty:
            ...
            _chart(aplicar_tema(fig_bot_box, 560))
        else:
    ...
    if not df_escola.empty:
        ...
        fig_escola.update_layout(
            ...
            legend=dict(
                orientation="h", yanchor="bottom", y=1.02,
                xanchor="right", x=1.0,
                font=dict(size=11, color=TEMA["texto"]),
                bgcolor="rgba(0,0,0,0)",
            ),
...
# ============================================================
def aba_territorial(df_ms_enriq, df_filt_ms_full=None, df_br=None, dep_selecionadas=None):
                ...
                if not evol.empty:
                    ...
                    for cre in cre_selecionadas:
                        ...
                        fig_evol_cre.add_trace(go.Scatter(
                            ...
                            name=f"{cre} (média)", mode="lines+markers",
                            line=dict(width=2.5)
                        ))
                    if pd.notna(medias_ref["ms"]):
                        fig_evol_cre.add_hline(y=medias_ref["ms"], line_dash="dash",
                                               line_color=AZUL_PRINCIPAL,
                                               annotation_text="Média MS", annotation_position="top left")
                    if pd.notna(medias_ref["br"]):
                        fig_evol_cre.add_hline(y=medias_ref["br"], line_dash="dot",
                                               line_color=COR_BRASIL,
                                               annotation_text="Média BR", annotation_position="bottom left")
                    fig_evol_cre.update_layout(
                        ...
                        yaxis=dict(title="Nota",
                                   range=range_dinamico(
                                       evol["Média"], padding=0.06,
                                       referencias=(medias_ref["ms"], medias_ref["br"]))),
                        hovermode="x unified"
                    )
                    _chart(aplicar_tema(fig_evol_cre, 450))
                else:
    ...
    if lista_cres and cre_selecionadas and "CRE" in df_dep_filt.columns:
        ...
        if not df_cre_filt.empty:
            ...
            fig_box_cre.update_layout(
                yaxis=dict(range=range_dinamico(
                    df_cre_filt[area], padding=0.05,
                    referencias=(medias_ref["ms"], medias_ref["br"]))),
            ...
            st.markdown("---")
            titulo_secao("Tabela completa de desempenho — CREs")
            st.dataframe(c, width="stretch", hide_index=True, height=400)
        else:
...
# ============================================================
# ABA 7 - EVOLUÇÃO HISTÓRICA
# ============================================================
def aba_evolucao_historica(df_filt_ms, df_br_filt):
    titulo_secao(
        "Evolução histórica 2019-2024",
        "Série temporal das notas médias por área e por dependência, com referência estadual e nacional."
    )

    area = st.selectbox("Área", options=list(AREAS.keys()),
                        format_func=nome_area_ext, key="area_series")
    deps_exibir = [d for d in ORDEM_DEP if d in df_filt_ms["DEP_ADM"].dropna().unique()]

    # Calcula médias de referência (MS Estadual e Brasil) para o período selecionado
    media_ms = df_filt_ms[df_filt_ms["DEP_ADM"] == "Estadual"][area].mean()
    media_br = df_br_filt[area].mean()

    _chart(fig_evolucao_area(
        df_filt_ms, area, deps_exibir,
        f"Evolução — {nome_area_ext(area)}",
        df_contexto=df_br_filt,
        media_ms_ref=media_ms,
        media_br_ref=media_br,
    ))

    titulo_secao("Painel completo — todas as áreas")
    grid = st.columns(2)
    for i, (col, nome) in enumerate(AREAS.items()):
        # Para o grid, também adicionamos as linhas de referência
        media_ms_grid = df_filt_ms[df_filt_ms["DEP_ADM"] == "Estadual"][col].mean()
        media_br_grid = df_br_filt[col].mean()
        fig = fig_evolucao_area(df_filt_ms, col, deps_exibir, nome_area_ext(col),
                                df_contexto=df_br_filt,
                                media_ms_ref=media_ms_grid,
                                media_br_ref=media_br_grid)
        fig.update_layout(height=280, title_font_size=12)
        with grid[i % 2]:
            _chart(fig)
...
def main():
    ...
    with tabs[4]:
        aba_evolucao_historica(df_filt_ms, df_br_filt)
    with tabs[5]:
        aba_territorial(df_ms_enriq, df_ms_enriq_todos, df_br_filt, dep_selecionadas)
    with tabs[6]:
