...
def aba_territorial(df_ms_enriq, df_filt_ms_full=None, df_br=None, dep_selecionadas=None, df_bruta_ms_enriq=None):
    ...
    if lista_cres and cre_selecionadas and "CRE" in df_dep_filt.columns:
        ...
        if not df_cre_filt.empty:
            ...
            cols_finais = ["CRE", "Estudantes", "Tx_Part_Efetiva", "Concluintes"]
            for k in AREAS.keys():
            ...
            st.dataframe(styled_cre, use_container_width=True, hide_index=True, height=520)
            ...
            if not df_cre_det.empty:
                ...
                for i, (key, nome) in enumerate(AREAS_COMPLETO.items()):
                    ...
                    fig_box_cre_det.add_trace(go.Box(
                        ...
                        jitter=0.3,
                        pointpos=-1.8,
                        marker=dict(size=4, opacity=0.4, color=cor_cre),
                        hoverinfo="skip",
                    ))
                    # Hover consolidado
                ...
                st.markdown(f"**Municípios da CRE {cre_detalhe} ({ano_detalhe_cre}) — {dep_detalhe_cre}**")
                ...
                if muni_col_cre in df_cre_det.columns:
                    ...
                    st.dataframe(styled_muni_cre, use_container_width=True, hide_index=True, height=520)
                else:
...
def aba_municipios(df_ms_enriq, df_filt_ms_full=None, df_br=None, dep_selecionadas=None, df_bruta_ms_enriq=None):
    ...
    if not df_muni_det.empty:
        ...
        if ano_detalhe == 2024 and ("NOME_ESCOLA" in df_muni_det.columns or "NO_ENTIDADE" in df_muni_det.columns):
            ...
            # Reordenar colunas: Escola → Concluintes → Participantes → Tx Part. Efetiva → [áreas]
            ...
            st.dataframe(styled_muni, use_container_width=True, hide_index=True, height=520)
    ...
    tabela_display["Taxa_Efetiva"] = tabela_display["Taxa_Efetiva"].apply(lambda x: f"{x:.2f}%".replace(".", ",") if pd.notna(x) else "—")
