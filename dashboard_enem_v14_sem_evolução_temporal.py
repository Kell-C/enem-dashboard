...
def aba_escolas_2024(df_ms_enriq_2024, ano=2024, df_br=None, df_bruta_ms=None, df_concluintes=None):
    ...
    if pd.notna(tx_ef_val) and pd.notna(conc_val) and conc_val > 0:
        kpi_card(kc4, f"Taxa de Part. Efetiva (de {fmt_int(int(insc_val))} inscritos)",
                 f"{tx_ef_val:.1f}%",
                 status="positivo" if tx_ef_val >= 80 else ("atencao" if tx_ef_val >= 60 else "critico"))
    else:
        kpi_card(kc4, "Taxa de Part. Efetiva", "—")
    ...
    # Calcular taxas com nomes autoexplicativos
    # Taxa de Inscrição = Inscritos no ENEM / Concluintes do 3º ano × 100
    tabela["Tx_Inscrição"] = (tabela["Inscritos"] / tabela["Concluintes"].replace(0, pd.NA) * 100)
    tabela["Tx_Inscrição"] = tabela["Tx_Inscrição"].apply(lambda x: round(x, 1) if pd.notna(x) else pd.NA)
    # Taxa de Participação Efetiva = Presentes nos 2 dias / Concluintes do 3º ano × 100
    tabela["Tx_Part_Efetiva"] = (tabela["Presentes"] / tabela["Concluintes"].replace(0, pd.NA) * 100)
    tabela["Tx_Part_Efetiva"] = tabela["Tx_Part_Efetiva"].apply(lambda x: round(x, 1) if pd.notna(x) else pd.NA)

    # Garantir que Concluintes, TURNOS e taxas existam
    ...
    tabela = tabela[["Escola", "Município", "Coordenadoria Regional", "TURNOS", 
                     "Concluintes", "Inscritos", "Presentes", 
                     "Tx_Inscrição", "Tx_Part_Efetiva", "Média", "Mediana"]]
    ...
    tabela["Tx_Inscrição_fmt"] = tabela["Tx_Inscrição"].apply(lambda x: f"{x:.1f}%" if pd.notna(x) else "—")
    tabela["Tx_Part_Efetiva_fmt"] = tabela["Tx_Part_Efetiva"].apply(lambda x: f"{x:.1f}%" if pd.notna(x) else "—")
    
    # Criar cópia para exibição com formatação
    tabela_display = tabela.copy()
    tabela_display["Tx_Inscrição"] = tabela_display["Tx_Inscrição_fmt"]
    tabela_display["Tx_Part_Efetiva"] = tabela_display["Tx_Part_Efetiva_fmt"]
    tabela_display = tabela_display.drop(columns=["Tx_Inscrição_fmt", "Tx_Part_Efetiva_fmt"])
    
    # ============================================================
    # ESTILIZAÇÃO AVANÇADA COM COLORAÇÃO DE LINHA
    # ============================================================
    def _colorir_taxa(val):
        """Colorir taxas com semáforo visual."""
        if val == "—":
            return "color: #9CA3AF;"
        try:
            num = float(val.replace("%", ""))
            if num >= 80:
                return "color: #059669; font-weight: 700; background-color: rgba(5, 150, 105, 0.08);"
            elif num >= 60:
                return "color: #D97706; font-weight: 700; background-color: rgba(217, 119, 6, 0.08);"
            else:
                return "color: #DC2626; font-weight: 700; background-color: rgba(220, 38, 38, 0.08);"
        except:
            return ""
    
    def _colorir_linha_por_taxa(row):
        """Colorir linha inteira baseado na Taxa de Participação Efetiva."""
        tx = row['Tx_Part_Efetiva']
        if pd.isna(tx) or tx == "—":
            return [''] * len(row)
        try:
            num = float(str(tx).replace("%", ""))
            if num >= 80:
                cor_fundo = 'background-color: rgba(5, 150, 105, 0.05);'
            elif num >= 60:
                cor_fundo = 'background-color: rgba(217, 119, 6, 0.05);'
            else:
                cor_fundo = 'background-color: rgba(220, 38, 38, 0.05);'
            return [cor_fundo] * len(row)
        except:
            return [''] * len(row)
    
    def _colorir_media(val):
        """Colorir médias com semáforo visual."""
        if pd.isna(val):
            return ""
        if val >= 600:
    ...
    def _destacar_escola(val):
        """Destacar nome da escola."""
        return "font-weight: 600; color: #1E3A5F;"
    
    def _centralizar_numeros(val):
        """Centralizar números."""
        return "text-align: center;"
    
    # Aplicar estilos
    styled = tabela_display.style
    
    # Aplicar coloração de linha baseada na Taxa de Part. Efetiva
    styled = styled.apply(_colorir_linha_por_taxa, axis=1)
    
    # Mapear cores para taxas
    styled = styled.map(_colorir_taxa, subset=["Tx_Inscrição"])
    styled = styled.map(_colorir_taxa, subset=["Tx_Part_Efetiva"])
    
    # Mapear cores para média
    styled = styled.map(_colorir_media, subset=["Média"])
    
    # Destacar escola
    styled = styled.map(_destacar_escola, subset=["Escola"])
    
    # Centralizar números
    styled = styled.map(_centralizar_numeros, subset=["Concluintes", "Inscritos", "Presentes", "Média", "Mediana"])
    
    # Formatar números com separador de milhar
    styled = styled.format({
        "Concluintes": lambda x: f"{int(x):,}".replace(",", ".") if pd.notna(x) else "—",
        "Inscritos": lambda x: f"{int(x):,}".replace(",", ".") if pd.notna(x) else "—",
        "Presentes": lambda x: f"{int(x):,}".replace(",", ".") if pd.notna(x) else "—",
        "Média": lambda x: f"{x:.2f}".replace(".", ",") if pd.notna(x) else "—",
        "Mediana": lambda x: f"{x:.2f}".replace(".", ",") if pd.notna(x) else "—",
    })
    ...
    styled = styled.set_properties(**{
        "font-size": "13px",
        "border-color": "#E5E7EB",
    }).set_properties(subset=["Escola", "Município", "Coordenadoria Regional"], **{
        "text-align": "left",
        "padding-left": "12px",
    }).set_properties(subset=["TURNOS"], **{
        "text-align": "center",
        "font-size": "11px",
        "color": "#4B5563",
    }).set_table_styles([
    ...
    <div style='font-size: 11px; color: #6B7280; margin-bottom: 10px;'>
        ...
        <span style='color: #D97706;'>■</span> 60-79% 
        <span style='color: #DC2626;'>■</span> <60% | 
        <b>Tx_Inscrição</b> = Inscritos no ENEM / Concluintes do 3º ano | 
        <b>Tx_Part_Efetiva</b> = Presentes nos 2 dias / Concluintes do 3º ano
    </div>
    """, unsafe_allow_html=True)
...
def aba_territorial(df_ms_enriq, df_filt_ms_full=None, df_br=None, dep_selecionadas=None, df_bruta_ms_enriq=None):
    ...
    if lista_cres and cre_selecionadas and "CRE" in df_dep_filt.columns:
        ...
        if not df_cre_filt.empty:
            ...
            # Montar tabela com médias de TODAS as áreas de conhecimento
            ...
            tabela_completa["Tx Part. Efetiva"] = (
                tabela_completa["Estudantes"] / tabela_completa["Concluintes"].replace(0, pd.NA) * 100
            ).round(1)

            # Arredondar colunas numéricas para 2 casas decimais
            for col in tabela_completa.columns:
                if col not in ("CRE", "Estudantes", "Tx Part. Efetiva", "Concluintes"):
                    tabela_completa[col] = tabela_completa[col].round(2)

            # Reordenar colunas: CRE, Estudantes, Tx Part. Efetiva, ... médias ...
            cols_medias = [c for c in tabela_completa.columns if c.startswith("Média ")]
            tabela_completa = tabela_completa[["CRE", "Estudantes", "Tx Part. Efetiva"] + cols_medias]

            tabela_completa = tabela_completa.sort_values(f"Média {AREAS['MEDIA_GERAL']}", ascending=False)

            # --- Médias de referência MS e BR por área para o ano ---
            medias_ms_ano = {}
            medias_br_ano = {}
            if ano_ref != "Todos os anos":
                for col in COLS_NOTAS + ["MEDIA_GERAL"]:
                    medias_ms_ano[col] = df_dep_filt[col].mean()
                if df_br is not None:
                    df_br_ref = df_br[(df_br["NU_ANO"] == ano_ref) & (df_br["DEP_ADM"] == dep_escolhido)]
                    for col in COLS_NOTAS + ["MEDIA_GERAL"]:
                        medias_br_ano[col] = df_br_ref[col].mean()

            # Função para colorir células por área (comparação MS/BR)
            def _colorir_tabela_cres(val, col_name):
                if pd.isna(val):
                    return ""
                # Identificar qual área esta coluna representa
                area_key = None
                ...
                ms_val = medias_ms_ano.get(area_key)
                br_val = medias_br_ano.get(area_key)

                if pd.notna(br_val) and val >= br_val:
                    return f"color: {COR_POSITIVO}; font-weight: 700;"
                elif pd.notna(ms_val) and val >= ms_val:
                    return f"color: {AZUL_PRINCIPAL}; font-weight: 700;"
                else:
                    return f"color: {COR_CRITICO}; font-weight: 700;"

            # Função para colorir Tx Part. Efetiva
            def _colorir_tx_part(val):
                if pd.isna(val):
                    return ""
                if val < 70:
                    return f"color: {COR_CRITICO}; font-weight: 700;"
            ...
            for col in tabela_completa.columns:
                if col.startswith("Média "):
                    styled = styled.map(lambda v, c=col: _colorir_tabela_cres(v, c), subset=[col])
                elif col == "Tx Part. Efetiva":
                    styled = styled.map(lambda v: _colorir_tx_part(v), subset=[col])

            # Criar tabela de exibição formatada como strings
            tabela_display = tabela_completa.copy()
            tabela_display["Estudantes"] = tabela_display["Estudantes"].apply(lambda x: f"{int(x):,}".replace(",", ".") if pd.notna(x) else "—")
            tabela_display["Tx Part. Efetiva"] = tabela_display["Tx Part. Efetiva"].apply(lambda x: f"{x:.2f}%".replace(".", ",") if pd.notna(x) else "—")
            for col in tabela_display.columns:
                if col.startswith("Média "):
                    tabela_display[col] = tabela_display[col].apply(lambda x: f"{x:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".") if pd.notna(x) else "—")

            styled = tabela_display.style
            for col in tabela_completa.columns:
                if col.startswith("Média "):
                    styled = styled.map(lambda v, c=col: _colorir_tabela_cres(float(v.replace(".", "").replace(",", ".")), c), subset=[col])
                elif col == "Tx Part. Efetiva":
                    styled = styled.map(lambda v: _colorir_tx_part(float(v.replace("%", "").replace(",", ".")) if v != "—" else None), subset=[col])
            ...
            st.markdown(
                ...
                "<b>Taxa de Participação Efetiva:</b> <span style='color:#C03A2B; font-weight:700;'>< 70%</span> = preocupante | "
                "<span style='color:#0F8A5F; font-weight:700;'>≥ 70%</span> = adequada"
                "</div>",
                unsafe_allow_html=True,
            )
            st.markdown(
                "<div style='font-size:12px; color:#6C757D; margin-bottom:8px;'>"
                "<b>Médias por área:</b> "
                "<span style='color:#0F8A5F; font-weight:700;'>≥ Média BR</span> | "
                "<span style='color:#003F7F; font-weight:700;'>≥ Média MS &lt; Média BR</span> | "
                "<span style='color:#C03A2B; font-weight:700;'>< Média MS</span>"
                "</div>",
                unsafe_allow_html=True,
            )
...
def aba_municipios(df_ms_enriq, df_filt_ms_full=None, df_br=None, dep_selecionadas=None, df_bruta_ms_enriq=None):
    ...
    if not df_muni_det.empty:
        ...
        if ano_detalhe == 2024 and ("NOME_ESCOLA" in df_muni_det.columns or "NO_ENTIDADE" in df_muni_det.columns):
            ...
            escolas_agg = df_muni_det.groupby(col_escola).agg(
                Estudantes=(area, "count"),
                **{f"Média_{k}": (k, "mean") for k in AREAS.keys()},
            ).reset_index()
            ...
            # Arredondar médias
            for k in AREAS.keys():
                escolas_agg[f"Média_{k}"] = escolas_agg[f"Média_{k}"].apply(lambda x: round(x, 1) if pd.notna(x) else pd.NA)

            # Renomear colunas para exibição com nomes completos e cores
            rename_map = {col_escola: "Escola", "Estudantes": "Inscritos", "Tx_Part_Efetiva": "Tx Part. Efetiva"}
            escolas_display = escolas_agg.rename(columns=rename_map)

            # Adicionar indicadores MS/BR integrados nas colunas de média de cada área
            for k in AREAS.keys():
                nome_area = AREAS_COMPLETO.get(k, k)

                def _fmt_media_com_setas(v, k=k):
                    if pd.isna(v):
                        return "—"
                    ms_str = ""
                    br_str = ""
                    if k in medias_ref_ms and pd.notna(medias_ref_ms[k]):
                        ms_str = "🟢↑" if v >= medias_ref_ms[k] else "🔴↓"
                    if k in medias_ref_br and pd.notna(medias_ref_br[k]):
                        br_str = "🟢↑" if v >= medias_ref_br[k] else "🔴↓"
                    parts = [f"{v:.1f}"]
                    if ms_str:
                        parts.append(ms_str + "MS")
                    if br_str:
                        parts.append(br_str + "BR")
                    return " ".join(parts)

                escolas_display[f"Média_{k}"] = escolas_display[f"Média_{k}"].apply(_fmt_media_com_setas)

            # Reordenar colunas: Escola → Concluintes → Tx Part. Efetiva → [áreas]
            colunas_ordem = ["Escola", "Concluintes", "Tx Part. Efetiva"]
            for k in AREAS.keys():
                colunas_ordem.append(f"Média_{k}")
            ...
            # Configurar exibição das colunas (column_config)
            col_config = {}
            col_config["Escola"] = st.column_config.TextColumn("Escola", width="large")
            col_config["Concluintes"] = st.column_config.NumberColumn("Concluintes", format="%d")
            col_config["Tx Part. Efetiva"] = st.column_config.TextColumn("Tx Part. Efetiva", width="medium")

            # Configurar colunas de áreas com cores via emoji no nome
            CORES_EMOJI = {
                "#7B8794": "⚪",
                "#0F8A5F": "🟢",
                "#E87722": "🟠",
            ...
            for k in AREAS.keys():
                nome_area = AREAS_COMPLETO.get(k, k)
                cor_area = CORES_AREAS.get(k, AZUL_PRINCIPAL)
                emoji_cor = CORES_EMOJI.get(cor_area, "🔵")
                col_config[f"Média_{k}"] = st.column_config.TextColumn(
                    f"{emoji_cor} {nome_area}",
                    width="medium",
                    help=f"Média em {nome_area}. 🟢↑MS/BR=Acima da média | 🔴↓MS/BR=Abaixo da média"
                )

            # Ordenar por média da área principal (usando valor numérico original)
            escolas_display = escolas_display.sort_values(f"Média_{area}", ascending=False, key=lambda col: pd.to_numeric(col.str.split().str[0], errors='coerce'))

            st.dataframe(escolas_display, column_config=col_config, hide_index=True, use_container_width=True)
    ...
    tabela_display = tabela[cols_display].copy()
    tabela_display["Taxa_Efetiva"] = tabela_display["Taxa_Efetiva"].apply(lambda x: f"{x:.1f}%" if pd.notna(x) else "—")

    # Colorir Taxa_Efetiva
    def _fmt_tx_muni(x):
        if x == "—":
            return x
        val = float(x.replace("%", ""))
    ...
    tabela_display["Taxa_Efetiva"] = tabela_display["Taxa_Efetiva"].apply(_fmt_tx_muni)

    # Integrar setas MS/BR em cada coluna de área
    for k in AREAS.keys():
        col_name = AREAS_COMPLETO[k]
        if col_name not in tabela_display.columns:
            continue
        ms_val = medias_ref_ms.get(k)
        br_val = medias_ref_br.get(k)

        def _fmt_media_geral(v, ms=ms_val, br=br_val):
            if pd.isna(v):
                return "—"
            parts = [f"{v:.1f}"]
            if ms is not None and pd.notna(ms):
                parts.append("🟢↑MS" if v >= ms else "🔴↓MS")
            if br is not None and pd.notna(br):
                parts.append("🟢↑BR" if v >= br else "🔴↓BR")
            return " ".join(parts)

        tabela_display[col_name] = tabela_display[col_name].apply(_fmt_media_geral)

    # Configurar colunas com cores das áreas
    CORES_EMOJI = {
        "#7B8794": "⚪", "#0F8A5F": "🟢", "#E87722": "🟠",
        "#1A56DB": "🔵", "#C03A2B": "🔴", "#C5A059": "🟡",
    }
    col_config_muni = {
        "Município": st.column_config.TextColumn("Município", width="medium"),
        "CRE": st.column_config.TextColumn("CRE", width="small"),
        "Concluintes": st.column_config.NumberColumn("Concluintes", format="%d"),
        "Taxa_Efetiva": st.column_config.TextColumn("Tx Part. Efetiva", width="medium"),
    }
    for k in AREAS.keys():
        nome_area = AREAS_COMPLETO.get(k, k)
        cor_area = CORES_AREAS.get(k, AZUL_PRINCIPAL)
        emoji_cor = CORES_EMOJI.get(cor_area, "🔵")
        col_config_muni[AREAS_COMPLETO[k]] = st.column_config.TextColumn(
            f"{emoji_cor} {nome_area}",
            width="medium",
            help=f"Média em {nome_area}. 🟢↑MS/BR=Acima | 🔴↓MS/BR=Abaixo"
        )

    st.dataframe(tabela_display, column_config=col_config_muni, hide_index=True, height=520, use_container_width=True)
