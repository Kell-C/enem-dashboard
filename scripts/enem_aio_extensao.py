"""Extensao 2016-2018: medias MS estadual a partir do historico AIO por escola."""
...
from enem_config import ANO_INICIAL, ANOS, ANOS_EXTENSAO_AIO, AREA_KEYS, NOTA_MAP, PASTA_DADOS

HISTORICO_CSV = PASTA_DADOS / "aio" / "enem_escolas_historico.csv"


def anos_extensao_aio() -> list[int]:
    return list(ANOS_EXTENSAO_AIO)
...
def _ano_usa_aio(ano: int) -> bool:
    return ano in ANOS_EXTENSAO_AIO and ano >= ANO_INICIAL


def medias_ms_por_ano() -> dict[int, dict]:
    """{ano: {geral, CN, CH, LC, MT, RED, n_escolas}} para anos sem microdado INEP."""
...
def participantes_estadual_por_ano() -> dict[int, int]:
    """Soma N_PARTICIPANTES (AIO) por ano, rede estadual MS — fallback 2016-2018."""
    hist = _carregar_ms_estadual()
...
def area_detail_web_extensao() -> dict[str, dict[str, dict]]:
    """Registros areaDetail para 2016-2018 (fonte AIO, sem histograma individual)."""
    hist = _carregar_ms_estadual()
...
def referencias_rows() -> list[dict]:
    """Linhas no formato referencias.parquet para merge no painel (2016-2018)."""
